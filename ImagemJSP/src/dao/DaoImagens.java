...
public class DaoImagens {
	...
	public void salvarOuAtualizar(Imagens imagen) throws Exception {
		try {
			if (imagen.getId() == null || imagen.getId() <= 0) {// insere
				String sql = "INSERT INTO imagens( miniatura, urlimagem, urlminiimg)VALUES ( ?, ?, ?);";
	...
	private void atualiza(Imagens imagem) throws Exception {
		String sql = "UPDATE imagens SET  miniatura=?, urlimagem=?, urlminiimg=?  where id = "
				+ imagem.getId();
	...
	public List<Imagens> consultaTodos() {
		...
		String sql = "select id , miniatura, urlminiimg FROM imagens order by id;";
	...
	private void constroiStatement(Imagens imagen, PreparedStatement insert)
		...
		insert.setString(1, imagen.getMiniatura());
		insert.setString(2, imagen.getUrlimagem());
		insert.setString(3, imagen.getUrlminiimg());
	}
